	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (reset == null) {
			...
			if (password.trim() == null || password.trim().length() == 0) {
				...
				} else if ((password2.trim() != null || password2.trim().length() > 0) && !password2.equals(password)) {
			...
			if (!errMessage.isEmpty()) {
				...
				if (request.getHeader("referer").indexOf("?") != -1) {
					response.sendRedirect(
							request.getHeader("referer").substring(0, request.getHeader("referer").indexOf("?"))
									+ "?Name=" + name + "&mail=" + email);
					// System.out.println("1");
					return;
				} else {
					String url = request.getHeader("referer") + "?";
					response.sendRedirect(url.substring(0, url.indexOf("?")) + "?Name=" + name + "&mail=" + email);
